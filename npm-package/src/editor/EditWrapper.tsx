...
type EditWrapperChanges = Map<
  string,
  { originalValue: string; newValue: string }
>
...
> {
  constructor(props: EditWrapperProps) {
    ...
    if (!((props.lexicon as any) instanceof Lexicon))
      throw new Error(
        `'lexicon' prop should be a Lexicon object, but it is: ${JSON.stringify(props.lexicon).substring(0, 50)}`
      )
  ...
  saveChanges = () => {
    ...
    const listOfChanges = [...this.state.unsavedChanges.entries()].map(
  ...
  render() {
    ...
    if (!this.allowEditing()) {
      return React.createElement(
        renderedComponent,
        { lexicon, ...renderedComponentProps },
        children
      )
    } else {
      ...
      return (
        <div className="EditWrapper">
          ...
          >
            <hgroup>
              ...
              <div className="position">
                {[
                  ...
                  <label
                    key={pos}
                    className={this.state.position == pos ? 'selected' : ''}
                  >
                    {icon}
                    <input
                      type="radio"
            ...
            {this.state.savingState == SavingState.Error && (
              <p className="error-message">{this.state.errorMessage}</p>
            )}
            <div
              className={`resizer resizer-${this.state.position}`}
