...
describe('Lexicon module', () => {
  ...
  beforeEach(() => {
    ...
    const lexObj = {
      ...
      en: {
        ...
        arrayOfObjects: [{ text: 'one' }, { text: 'two' }],
      ...
      es: {
        ...
        arrayOfObjects: [{ text: 'uno' }, { text: 'dos' }],
  ...
  describe('get()', () => {
    ...
    test('returns correct values for "string" key', () => {
      expect(lex.get('string', { color: 'blue' })).toEqual(
  ...
  describe('locale()', () => {
    ...
    test('locale and nested Lexicons work together', () => {
      expect(
        lex.locale('es').subset('subLex').locale('en').get('subFoo')
      ).toEqual('SUB FOO')
    })
  })
  ...
  describe('keys()', () => {
    ...
    test('works with different locales', () => {
      expect(lex.locale('es').keys()).toEqual([
        'foo',
        'nested.wom',
        'arrayOfObjects.0.text',
        'arrayOfObjects.1.text',
    ...
    test('keys() for subsets only returns keys under that subset', () => {
      expect(lex.subset('nested').keys()).toEqual(['wom'])
    })
  })
