...
RSpec.describe ApiController, type: :controller do
  describe "#update" do
    let(:mock_response) do
      double(HTTParty::Response, code: 200, parsed_response: nil)
    end
    ...
    before do
      ...
      @original_content = { key: "old value", parent: { child: "old child" } }
      ...
      @lexicon_changes = {
        changes: [{ filename: @tempfile, key: "key", newValue: "new value" }],
      }

      @user =
        User.create!(
          email: "blaaa@example.com",
          password: "super.blah.foofy.foo",
        )
      @client_app =
        ClientApp.create!(
    ...
    context "when logged in" do
      ...
      it 'alerts changes via Slack when "slack_workflow_url" is filled in' do
        @token =
          ApiController.lexicon_server_token(@user, @client_app_with_slack)
        request.headers["Authorization"] = "Bearer #{@token}"
      ...
      it 'does not talk to Slack when "slack_workflow_url" is empty string' do
        @token =
          ApiController.lexicon_server_token(
            @user,
            @client_app_with_slack_empty,
          )
        request.headers["Authorization"] = "Bearer #{@token}"
    ...
    context "when server token is for a particular ClientApp at GitHub" do
      before do
        ...
        allow(LexServer::Adapter).to receive(:configure).and_return(
          double("sample lexicon adapter", read: {}, write_changed_files: nil),
        )
        ...
        expect(response.parsed_body).to eq(
          {
            "successful" => false,
            "error" =>
              "Lexicon Server: Invalid token from client app: Signature has expired",
          },
        )
      end
      ...
      it "writes to filename set in ClientApp" do
        ...
        expect(lexicon_adapter).to receive(:write_changed_files).with(
          "blaaa@example.com via Lexicon Editor",
          anything,
        )
        put(:update, params: @lexicon_changes)
      end
  ...
  describe ".cors_friendly_origin" do
    ...
    it "excludes port when its 443 so CORS doesnt get upset, i.e. the strings match exactly" do
      ...
      expect(ApiController.cors_friendly_origin(url)).to eq(
        "https://lexicon.nitid.co",
      )
    end

    it "keeps good URL for staging lexicon server" do
      url = "https://lexicon-staging.onrender.com/mydir"
      expect(ApiController.cors_friendly_origin(url)).to eq(
        "https://lexicon-staging.onrender.com",
      )
    end
  end
end
